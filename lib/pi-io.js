...
PiIO.prototype._pingRead = function(pinData, callback) {
  var rangeFinder = pinData.custom.rangeFinder,
    gpio;

  if (!rangeFinder) {
    gpio = new Gpio(pinData.gpioNo, {
      mode: Gpio.OUTPUT,
      pullUpDown: Gpio.PUD_OFF,
      alert: true
    });

    rangeFinder = pinData.custom.rangeFinder = new RangeFinder(gpio);
  }

  rangeFinder.pingRead(callback);
};
